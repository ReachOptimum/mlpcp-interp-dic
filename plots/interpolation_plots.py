...
x_ori_eyy = []
x_ori_exy = []
with open(X_TRAIN, mode='r') as file:
    reader = csv.reader(file)
    # header = next(reader)
    row = next(reader)
...
# Interpolated DIC exx, eyy and exy values
# Interpolated exx, eyy and exy values
x_int_dic_exx = []
x_int_dic_eyy = []
x_int_dic_exy = []
with open(DIC_INTERP, 'r') as file:
    reader = csv.reader(file)
    header = next(reader)
    row = next(reader)
    for i in range(len(row) - (int(len(row) / 20) - 2), len(row), 3):
        x_int_dic_exx.append(float(row[i]))
        x_int_dic_eyy.append(float(row[i+1]))
        x_int_dic_exy.append(float(row[i+2]))
x_int_dic_exx = np.array(x_int_dic_exx)
x_int_dic_eyy = np.array(x_int_dic_eyy)
x_int_dic_exy = np.array(x_int_dic_exy)

# Update global color scale
# global_min = min(global_min, np.nanmin(x_int_dic_exx))
# global_max = max(global_max, np.nanmax(x_int_dic_exx))
...
for grid in GRIDS:
    ...
    for method in METHODS:
        ...
        # --- absolute differences (interpolated - interpolated DIC) ---
        x_diff_exx = np.abs(x_int_exx - x_int_dic_exx)
        x_diff_eyy = np.abs(x_int_eyy - x_int_dic_eyy)
        x_diff_exy = np.abs(x_int_exy - x_int_dic_exy)

        print(f"Avarage abs diff exx: {np.nanmean(x_diff_exx)}")
        print(f"Avarage abs diff eyy: {np.nanmean(x_diff_eyy)}")
        print(f"Avarage abs diff exy: {np.nanmean(x_diff_exy)}")

        x_int_dic_exy = x_int_dic_exy * 2
        
        rows, cols = 3, 3

        fig_width_in = 13.8 / 2.54  # Convert cm to inches
        subplot_size = fig_width_in / 3  # Keep subplots square
        num_params = 9  # Number of subplots

        fig_height_in = rows * subplot_size  # Maintain square aspect ratio
        fig, axes = plt.subplots(rows, cols, figsize=(fig_width_in, fig_height_in*1.1))
        # axes = axes.flatten()
        ...
        plt.subplots_adjust(
            ...
            bottom=0.12, # Adjust bottom margin
            wspace=0.5,  # Increase horizontal space between plots
        ...
        positions = [
            (0.08, 0.988), (0.42, 0.988), (0.76, 0.988),
            (0.08, 0.683), (0.42, 0.683), (0.76, 0.683),
            (0.08, 0.377), (0.42, 0.377), (0.76, 0.377)
        ]
        ...
        def add_corner_label(ax, text):
            ax.text(
                ...
                ha="right", va="top",
                # fontsize=7,
                # bbox=dict(boxstyle="round,pad=0.1", facecolor="white", edgecolor="black", linewidth=0.6),
                zorder=10,
            )
        ...
        global_min = min(np.nanmin(x_int_exx), np.nanmin(x_int_eyy), np.nanmin(x_int_exy),
                        ...
                        np.nanmin(x_int_dic_exx), np.nanmin(x_int_dic_eyy), np.nanmin(x_int_dic_exy))
        global_max = max(np.nanmax(x_int_exx), np.nanmax(x_int_eyy), np.nanmax(x_int_exy),
                        ...
                        np.nanmax(x_int_dic_exx), np.nanmax(x_int_dic_eyy), np.nanmax(x_int_dic_exy))
        
        # print(global_min, global_max)

        # Define the data pairs for each row
        data_pairs = [
            (x_int_exx, x_ori_exx, x_int_dic_exx),
            (x_int_eyy, x_ori_eyy, x_int_dic_eyy),
            (x_int_exy, x_ori_exy, x_int_dic_exy)
        ]
        ...
        for row_idx, (x_int_left, x_dic, x_int_right) in enumerate(data_pairs):
            ...
            # Mid column (DIC)
            scatter_dic = axes[row_idx, 1].scatter(
                x_centroids, y_centroids, c=x_dic, cmap='jet', s=1, alpha=0.9, edgecolors='none',
            ...
            # Right column (interpolated DIC)
            scatter_int_right = axes[row_idx, 2].scatter(
                x_coords, y_coords, c=x_int_right, cmap='jet', s=size, alpha=0.9, edgecolors='none',
                vmin=global_min, vmax=global_max
            )
            # axes[row_idx, 1].set_title(f'{titles[row_idx]} - Original')
            axes[row_idx, 2].set_xlabel('x [mm]')
            axes[row_idx, 2].set_ylabel('y [mm]')
            axes[row_idx, 2].set_aspect("equal", adjustable="box")

            # equal scales and ticks on both axes
            axes[row_idx, 2].set_xlim(x_min, x_max)
            axes[row_idx, 2].set_ylim(y_min, y_max)
            axes[row_idx, 2].set_xticks(x_ticks)
            axes[row_idx, 2].set_yticks(y_ticks)
            axes[row_idx, 2].set_aspect("equal")  # force equal scaling

            label_txt = row_legend_labels[row_idx]

            # after axes[row_idx, 0] is set up
            add_corner_label(axes[row_idx, 0], label_txt)

            # after axes[row_idx, 1] is set up
            add_corner_label(axes[row_idx, 1], label_txt)

            # after axes[row_idx, 2] is set up
            add_corner_label(axes[row_idx, 2], label_txt)
        ...
        cax = fig.add_axes([0.2, 0.035, 0.6, 0.02])  # [left, bottom, width, height]
        cbar = fig.colorbar(scatter_dic, cax=cax, orientation='horizontal')
        ...
        plt.savefig(os.path.join(PLOT, f"interp_{grid}_{method}.svg"), format="svg")
        plt.close()
